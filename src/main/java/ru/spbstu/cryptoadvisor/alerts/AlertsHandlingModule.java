package ru.spbstu.cryptoadvisor.alerts;
...
import ru.spbstu.cryptoadvisor.api.binance.BingXService;
import ru.spbstu.cryptoadvisor.api.rabbitmq.RabbitConfig;
import ru.spbstu.cryptoadvisor.api.rabbitmq.RabbitMQService;
import ru.spbstu.cryptoadvisor.telegram.TelegramBotService;
...
public class AlertsHandlingModule implements InitializingBean {
    ...
    private void initQueueFromDb() {
        ...
        Result<?> trackedRecords = dsl
            ...
            .on(
                field("tc.crypto_currency_id").eq(field("c.crypto_currency_id"))
            )
            .fetch();
        ...
        for (Record record : trackedRecords) {
            ...
            Integer trackedCurrencyId = record.get(
                field("tc.tracked_currency_id"),
                Integer.class
            );
    ...
    public void processAlertCheck(AlertCheckMessage task) {
        ...
        if (task.getAlertId() != null) {
            ...
            Integer exists = dsl
                ...
                .where(
                    field("tracked_currency_id").eq(task.getTrackedCurrencyId())
                )
                .fetchOne(0, Integer.class);
            if (exists == null || exists == 0) {
                log.info(
    ...
    public void cleanExpiredAlerts() {
        ...
        dsl
            .deleteFrom(table("alert_history"))
    ...
    public String getActiveAlertsStatusString() {
        ...
        Result<?> trackedRecords = dsl
            .select(field("tc.tracked_currency_id"), field("c.symbol"))
            .from(table("tracked_currency").as("tc"))
            .join(table("crypto_currency").as("c"))
            .on(
                field("tc.crypto_currency_id").eq(field("c.crypto_currency_id"))
    ...
    public List<String> getRecentAlerts(Long userId) {
        ...
        return dsl
            ...
            .on(
                field("ah.tracked_currency_id").eq(
                    field("tc.tracked_currency_id")
                )
            )
            ...
            .and(
                field("ah.date_record").greaterThan(
                    Timestamp.valueOf(
                        LocalDateTime.now().minusDays(ALERT_EXPIRY_DAYS)
                    )
                )
